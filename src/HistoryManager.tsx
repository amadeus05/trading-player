...
export function HistoryManager({onOpen}:{onOpen:(market:{id:string;name:string;candles:Candle[]})=>void}){
  ...
  const columns=[{title:"Рынок",render:(_:unknown,r:CatalogItem)=><><b>{r.symbol}</b> <Tag>{r.category}</Tag></>},{title:"Покрытие",render:(_:unknown,r:CatalogItem)=>`${date(r.from)} — ${date(r.to-1)}`},{title:"5m свечей",dataIndex:"candles",render:(v:number)=>v.toLocaleString()},{title:"Размер",dataIndex:"bytes",render:(v:number)=>`${(v/1024/1024).toFixed(1)} MB`},{title:"",align:"right" as const,render:(_:unknown,r:CatalogItem)=><Button type="primary" ghost className="history-open-btn" icon={<BarChart3 size={14}/>} onClick={()=>openRange(r.category,r.symbol,r.from,r.to)}>Открыть</Button>}];
  return <><Button icon={<Database size={16}/>} onClick={()=>setOpen(true)}>История</Button><Modal title="История Bybit" open={open} width={850} onCancel={()=>setOpen(false)} footer={null} destroyOnHidden>
    <Form form={form} component={false} initialValues={{category:"linear",symbol:"BTCUSDT",range:[dayjs().subtract(30,"day"),dayjs().subtract(1,"day")]}}>
      <div className="history-form">
        ...
        <Button className="history-form-download" type="primary" icon={<Download size={15}/>} loading={loading} onClick={download}>Загрузить</Button>
      </div>
    </Form>
    ...
    {!catalog.length?<Alert type="info" showIcon title="Локальной истории пока нет"/>:<Table className="history-catalog-table" rowKey={r=>`${r.category}:${r.symbol}`} dataSource={catalog} columns={columns} pagination={false} size="small"/>}
  </Modal></>;
}
