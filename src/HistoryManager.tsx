...
export function HistoryManager({onOpen}:{onOpen:(market:{id:string;name:string;candles:Candle[]})=>void}){
  ...
  const columns=[{title:"Рынок",render:(_:unknown,r:CatalogItem)=><><b>{r.symbol}</b> <Tag>{r.category}</Tag></>},{title:"Покрытие",render:(_:unknown,r:CatalogItem)=>`${date(r.from)} — ${date(r.to-1)}`},{title:"5m свечей",dataIndex:"candles",render:(v:number)=>v.toLocaleString()},{title:"Размер",dataIndex:"bytes",render:(v:number)=>`${(v/1024/1024).toFixed(1)} MB`},{title:"",render:(_:unknown,r:CatalogItem)=><Button size="small" onClick={()=>openRange(r.category,r.symbol,r.from,r.to)}>Открыть историю</Button>}];
