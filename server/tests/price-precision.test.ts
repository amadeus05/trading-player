...
test("ETC-like prices around 6.17 keep at least 4 decimals", () => {
  const candles = [6.16, 6.17, 6.18, 6.19].map((close) => bar(close, 0.01));
  assert.ok(inferPricePrecision(candles) >= 4);
});
...
test("ETC-like draft does not park TP/SL on coarse 0.01 scale ticks", () => {
  const candles = Array.from({ length: 20 }, () => bar(6.17, 0.008));
  const { tp, sl } = buildInitialProtectionPrices(
    "LONG",
    6.17,
    { from: 6.155, to: 6.215 },
  ...
  assert.ok(tp < 6.20, `tp snapped to scale: ${tp}`);
  assert.ok(sl > 6.16, `sl snapped to scale: ${sl}`);
  assert.ok(tp > 6.17);
  assert.ok(sl < 6.17);
});
